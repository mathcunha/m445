...
public class FunctionHamming implements MathFunction {
	...
	public Double eval(State state) {
		int count = 0 ;
		BinaryCoordinate coordinate = (BinaryCoordinate)state.getCoordinate().get(0);
		boolean[] array = null;
		switch (coordinate.getValue().length) {
		case 12:
			array = ARRAY_4_3;
			break;
		case 108:
		...
		for (int i = 0; i < array.length; i++) {
			if(coordinate.getValue()[i] != array[i]){
				count++;
			}
		}
		Double ret = (new Integer(coordinate.getValue().length - count)).doubleValue();		
